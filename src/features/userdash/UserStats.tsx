...
const UserStats = () => {
  ...
  const { data: todayData, refetch } = useSingleEmployeeMealActivity(
    today,
    "1"
  );
  useEffect(() => {
    if (todayData) {
      const lunch =
        todayData[0].employee_details?.[0].meal?.[0].meal_status?.[0].status ??
        false;
      const snacks =
        todayData[0].employee_details?.[0].meal?.[1].meal_status?.[0].status ??
        false;
  ...
  return (
    ...
    <div className="flex">
      <div className="card bg-base-50 shadow rounded-box grid flex-grow place-items-center p-4 px-6 mr-2">
        ...
        <div
          className={`stat-value text-3xl ${
            lunchStatus ? "text-green-600" : "text-red-600"
          }`}
        >
          {lunchStatus ? "ON" : "OFF"}
        </div>
      ...
      <div className="card bg-base-50 shadow rounded-box grid flex-grow place-items-center p-4 px-6">
        <div className="stat-title text-zinc-500">{`Today's Snacks`}</div>
        <div
          className={`stat-value text-3xl ${
            snacksStatus ? "text-green-600" : "text-red-600"
          }`}
        >
          {snacksStatus ? "ON" : "OFF"}
        </div>
