...
const MealPlanTable = () => {
  ...
  const [endDate, setEndDate]= useState(dayjs(startDate).add(6,"day").format("YYYY-MM-DD"));
  ...
  const changeWeek = (direction: "prev" | "next") => {
    setStartDate((prevDate) => {
      const newStartDate = dayjs(prevDate).add(direction === "next" ? 7 : -7, "day").format("YYYY-MM-DD");
  ...
  const handleCopyMealsFromPreviousWeek = async () => {
    const previousWeekStart = dayjs(startDate).subtract(7, "day").format("YYYY-MM-DD");
  
    console.log("📅 Copying meals from:", previousWeekStart, "to", dayjs(previousWeekStart).add(6, "day").format("YYYY-MM-DD"));
    ...
    try {
      ...
      const prevWeekMeals = await request({
        ...
        useAuth: true,
      }) as { date: string; menu: { meal_type: string; food: string }[] }[] | null;
      ...
      notificationToast("Successfully copied meals from the previous week","success");
              ...
              {mealData.length === 0 ? (
                ...
                mealData.map((row) => (
                  <tr key={row.date} className="border text-center">
                    <td className="border p-2">{dayjs(row.date).format("ddd, DD MMM")}</td>
          ...
          <div className="flex justify-end mt-4">
            {!isEditing ? (
              ...
              <>
                ...
                <button onClick={() => { setEditedData(mealData); setIsEditing(false); }} className="bg-gray-500 text-white px-4 py-2">
                  Cancel
                </button>
              </>
            )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg">
            <h2 className="text-xl font-bold mb-4">Copy Meals from Previous Week</h2>
            <p>Are you sure you want to copy the meals from the previous week?</p>
            <div className="mt-4 flex justify-end">
              <button onClick={() => setIsModalOpen(false)}
                className="bg-gray-500 text-white px-4 py-2 mr-2">Cancel</button>
              <button 
               onClick={() => {
